...
test('组合顺序保持 body/outfit 在下、hair 在上，并按 64×64 注册帧',()=>{
  const order:string[]=[],keys=new Set(['player-male-body-v3','PLAYER_M_HAIR_01']);
  ...
  composeHairTextures({textures} as any,[hairConfigs[0]]);assert.deepEqual(order,['player-male-body-v3','PLAYER_M_HAIR_01']);
});
