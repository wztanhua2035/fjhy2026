...
test('组合顺序保持 body/outfit 在下、hair 在上，并按 64×64 注册帧',()=>{
  const order:string[]=[],keys=new Set(['player-male-body-v4','PLAYER_M_FACE_01','PLAYER_M_HAIR_01']);
  ...
  composeHairTextures({textures} as any,[hairConfigs[0]]);assert.deepEqual(order,['player-male-body-v4','PLAYER_M_FACE_01','PLAYER_M_HAIR_01']);
});
...
test('更换 Face 只切换合成贴图，null Headwear 不增加图层',()=>{
  const order:string[]=[],keys=new Set(['player-female-body-v4','PLAYER_F_HAIR_02','PLAYER_F_FACE_01','PLAYER_F_FACE_02','PLAYER_F_FACE_03']);
  ...
  assert.deepEqual(order.slice(6),['player-female-body-v4','PLAYER_F_FACE_03','PLAYER_F_HAIR_02']);
