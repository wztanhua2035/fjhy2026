...
test('男女三完整发型、三整套服装均可建角，重复请求不重复发钱',async()=>{
  ...
  for(const gender of ['MALE','FEMALE'] as const){
    ...
    for(let index=0;index<3;index++){
      const player=await repo.login(`${gender}-${index}`),body={requestId:randomUUID(),gender,hairId:options.hairs[index].id,outfitId:options.outfits[index].id};
      ...
      assert.equal('skinToneId' in (saved.appearance??{}),false);
...
test('建角不依赖旧发色/上衣色/下装色，缺省项回退合法选项，伪造 ID 被拒',async()=>{
  const options=availableStarterLookOptions({appearances:initialWorld.appearances,colors:initialWorld.colors},'FEMALE',{hairId:'HAIR_ABSENT',outfitId:'OUTFIT_ABSENT'});
  assert.deepEqual(options.selection,{hairId:'F_HAIR_01',outfitId:'F_OUTFIT_01'});
...
test('创建 API 的共享客户端只发送新字段，重登 bootstrap 保留外观',async()=>{
  ...
  try{
    ...
    const body={requestId:randomUUID(),gender:'FEMALE',hairId:'F_HAIR_03',outfitId:'F_OUTFIT_02'};
    ...
    await controller.create('FEMALE',{hairId:'F_HAIR_01',outfitId:'F_OUTFIT_01'});
...
test('旧外观字段原样保留，新语义映射到安全默认选项',()=>{
  const legacy={gender:'FEMALE' as const,baseAvatarId:'FEMALE_04',hairStyleId:'HAIR_FEMALE_03',hairColorId:'CHESTNUT',topStyleId:'TOP_FEMALE_02',topColorId:'ROSE',bottomStyleId:'BOTTOM_FEMALE_01',bottomColorId:'CREAM',shoesId:'SHOES_FEMALE_01',accessoryIds:[]};
...
test('未来 NPC 可直接配置完整 Hair/Outfit 而无需旧式上下装字段',()=>{
  ...
  world.npcs[0].appearance={gender:'MALE',hairId:'M_HAIR_01',outfitId:'M_OUTFIT_01',accessoryIds:[]};
