...
import {scaleSafeInsets,wechatRequestHeaders} from '../apps/wechat-game/src/wechat-platform.js';

test('微信 DEV 全部营业仅按构建开关附加请求头',()=>{assert.deepEqual(wechatRequestHeaders('token',false),{'Content-Type':'application/json',Authorization:'Bearer token'});assert.deepEqual(wechatRequestHeaders('token',true),{'Content-Type':'application/json',Authorization:'Bearer token','X-Debug-Open-All':'1'});});
test('微信安全区按 16:9 设计画布等比换算',()=>{assert.deepEqual(scaleSafeInsets({windowWidth:844,windowHeight:390,safeArea:{left:44,right:820,top:0,bottom:369}},960,540),{left:44/844*960,right:24/844*960,top:0,bottom:21/390*540});assert.deepEqual(scaleSafeInsets({},960,540),{left:0,right:0,top:0,bottom:0});});

test('客户端 stand 直接拒绝每个正式建筑局部碰撞中心点',()=>{const c=new GameController(async()=>({}));c.boot={player:{id:'p',nickname:'旅人',cash:120,stamina:100,status:'ACTIVE',sceneId:'STREET_BAISHI_01',x:23,y:23,appearance:{} as any,inventory:{},storage:{},life:{energy:100,sleep:null,lastEffectiveSleepAt:null,lastSleepResult:null},cosmetics:[],ledger:[],tradeCounts:{},metNpcs:[]},serverTime:'',worldVersion:1,configVersion:1,assetVersion:1,assetManifest:'',colors:{},appearances:[],features:{}};c.view=sceneView(initialWorld,'STREET_BAISHI_01',new Date('2026-09-11T12:00:00Z'));for(const [buildingId,rects] of Object.entries(baishiBuildingObjectCollision))for(const [index,rect] of rects.entries())assert.equal(c.stand(rect.x+rect.width/2,rect.y+rect.height/2),false,`${buildingId}#${index+1} 中心点必须不可站立`);});
...
test('任务追踪器从 Quest 配置生成步骤、目标、奖励和完成状态',()=>{
  ...
  c.boot={player:{id:'p',nickname:'旅人',cash:108,stamina:100,status:'ACTIVE',sceneId:'STREET_BAISHI_01',x:12,y:15,appearance:{} as any,inventory:{RICE_01:1},storage:{},life:{energy:100,sleep:null,lastEffectiveSleepAt:null,lastSleepResult:null},cosmetics:[],ledger:[accepted,buy],tradeCounts:{},metNpcs:[]},serverTime:'',worldVersion:1,configVersion:1,assetVersion:1,assetManifest:'',colors:{},appearances:[],features:{}};
...
test('通用商店视图从场景配置生成商品、余额、持有量和双向价格',()=>{
  const c=new GameController(async()=>({}));
  c.boot={player:{id:'p',nickname:'旅人',cash:108,stamina:100,status:'ACTIVE',sceneId:'INTERIOR_B_GROCERY',x:12,y:9,appearance:{} as any,inventory:{RICE_01:1},storage:{},life:{energy:100,sleep:null,lastEffectiveSleepAt:null,lastSleepResult:null},cosmetics:[],ledger:[],tradeCounts:{},metNpcs:[]},serverTime:'',worldVersion:1,configVersion:1,assetVersion:1,assetManifest:'',colors:{},appearances:[],features:{}};
