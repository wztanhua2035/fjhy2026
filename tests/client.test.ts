...
test('首次交谈显示已认识反馈',async()=>{let c:GameController;c=new GameController(async()=>({player:{...c.player,metNpcs:['NPC_002']},dialogue:'很高兴见到你'}));c.token='token';c.boot={player:{id:'p',nickname:'旅人',cash:120,stamina:100,status:'ACTIVE',sceneId:'STREET_BAISHI_01',x:16,y:22.5,appearance:{} as any,inventory:{},cosmetics:[],ledger:[],tradeCounts:{},metNpcs:[]},serverTime:'',worldVersion:1,configVersion:1,assetVersion:1,assetManifest:'',colors:{},appearances:[],features:{}};c.view={scene:{id:'STREET_BAISHI_01'},plots:[],buildings:[],npcs:[{id:'NPC_002',name:'阿青'}]} as any;await c.write('/v1/npc/talk',{npcId:'NPC_002'});assert.match(c.message,/初次结识：阿青/);assert.match(c.message,/关系记录：已认识/);});
