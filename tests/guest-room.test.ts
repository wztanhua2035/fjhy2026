...
test('临时房尺寸、美术、四件家具碰撞和独立可达交互点',async()=>{
  assert.equal(scene.width,12);assert.equal(scene.height,10);assert.equal(scene.tileSize,32);
  assert.equal(scene.width*scene.tileSize,384);assert.equal(scene.height*scene.tileSize,320);
  ...
  for(const id of ['GUEST_BED','GUEST_CHEST','GUEST_WARDROBE','GUEST_DESK']){
    ...
    assert.equal(reachable({x:6,y:7},zone.interactionPoint),true,id);
  }
  ...
  assert.equal(reachable({x:6,y:7},{x:6,y:8}),true);
  const base='apps/admin/public/scene-layers/baishi/interiors/';
  for(const file of ['interior_guest_room_v1.png','interior_guest_room_fg_v1.png']){
    const png=await readFile(base+file);assert.equal(png.readUInt32BE(16),384);assert.equal(png.readUInt32BE(20),320);
  }
});
...
test('四个物件服务端按共享交互点校验，不能远程查看',async()=>{
  ...
  Object.assign(repo.players.get(p.id)!,{x:6,y:7});
...
test('临时房双向 portal、开场锁定、关系预置、重登和重新开始',async()=>{
  ...
  Object.assign(repo.players.get(p.id)!,{x:6,y:8});
...
test('共享 GameController 自动逐句开场，期间不能移动，结束后恢复且只播一次',async()=>{
  ...
  await c.advanceDialogue();c.x=6;c.y=8;await c.interact();
