...
const appearance=(gender='FEMALE')=>({requestId:randomUUID(),gender,faceId:`${gender==='MALE'?'M':'F'}_FACE_01`,hairId:`${gender==='MALE'?'M':'F'}_HAIR_01`,outfitId:`${gender==='MALE'?'M':'F'}_OUTFIT_01`,profile:testProfile()});
...
async function completeOpening(post:(url:string,payload:object)=>Promise<any>){
 ...
 for(const [x,y] of [[3,6],[7,6],[7,10],[12,10]])await post('/v1/player/move',{x,y});
 await post('/v1/player/move',{x:12,y:18});
 await post('/v1/player/move',{x:12,y:18});await post('/v1/world/portal',{portalId:'EXIT_B_INN'});
}
...
test('DEV 全部营业请求级覆盖进入、场景 NPC 与恢复正常规则',async()=>{
 const closedAt=()=>new Date('2026-09-05T15:00:00Z'),repo=new MemoryRepository(),app=await buildApp(repo,env,{now:closedAt});
 try{
  const login=(await app.inject({method:'POST',url:'/v1/auth/dev',payload:{account:'open-all'}})).json(),headers={authorization:`Bearer ${login.token}`},debugHeaders={...headers,'x-debug-open-all':'1'};
  await new GameService(repo,closedAt).action(login.player.id,'create',appearance());
  const enter=async(plotId:string,entranceId:string,requestHeaders:Record<string,string>)=>app.inject({method:'POST',url:'/v1/world/enter',headers:requestHeaders,payload:{requestId:randomUUID(),plotId,entranceId}});
  const cases=[['P_BAISHI_004','ENT_BAISHI_SALON_W','B_SALON','INTERIOR_B_SALON','NPC_SALON_HAIRDRESSER',44,20],['P_BAISHI_005','ENT_BAISHI_CLOTH_S','B_CLOTH','INTERIOR_B_CLOTH','NPC_CLOTH_SHOPKEEPER',34,37.6]] as const;
  for(const [plotId,entranceId,_buildingId,interior,npcId,x,y] of cases){
   Object.assign(repo.players.get(login.player.id)!,{sceneId:'STREET_BAISHI_01',x,y});
   const closed=await enter(plotId,entranceId,headers);assert.equal(closed.statusCode,400,closed.body);assert.match(closed.body,/店铺已打烊/);
   const opened=await enter(plotId,entranceId,debugHeaders);assert.equal(opened.statusCode,200,opened.body);assert.equal(opened.json().player.sceneId,interior);
   const scene=await app.inject({url:`/v1/world/scenes/${interior}`,headers:debugHeaders});assert.equal(scene.statusCode,200,scene.body);assert.ok(scene.json().npcs.some((npc:any)=>npc.id===npcId));
   const npc=initialWorld.npcs.find(candidate=>candidate.id===npcId)!;Object.assign(repo.players.get(login.player.id)!,{x:npc.x,y:npc.y});
   const talked=await app.inject({method:'POST',url:'/v1/npc/talk',headers:debugHeaders,payload:{requestId:randomUUID(),npcId}});assert.equal(talked.statusCode,200,talked.body);
  }
  Object.assign(repo.players.get(login.player.id)!,{sceneId:'STREET_BAISHI_01',x:34,y:37.6});
  const restored=await enter('P_BAISHI_005','ENT_BAISHI_CLOTH_S',headers);assert.equal(restored.statusCode,400,restored.body);
  assert.deepEqual(initialWorld.buildings.find(b=>b.id==='B_CLOTH')!.openingHours,['08:00','20:30']);assert.deepEqual(initialWorld.buildings.find(b=>b.id==='B_SALON')!.openingHours,['08:00','20:30']);
 }finally{await app.close();}
});

test('production 忽略全部营业调试请求头',async()=>{
 const closedAt=()=>new Date('2026-09-05T15:00:00Z'),repo=new MemoryRepository(),production={...env,mode:'production',appEnv:'PROD',allowDevAuth:false},app=await buildApp(repo,production,{now:closedAt});
 try{const p=await repo.login('production-open-all');await new GameService(repo,closedAt).action(p.id,'create',appearance());Object.assign(repo.players.get(p.id)!,{sceneId:'STREET_BAISHI_01',x:34,y:37.6});const response=await app.inject({method:'POST',url:'/v1/world/enter',headers:{authorization:`Bearer ${app.jwt.sign({sub:p.id})}`,'x-debug-open-all':'1'},payload:{requestId:randomUUID(),plotId:'P_BAISHI_005',entranceId:'ENT_BAISHI_CLOTH_S'}});assert.equal(response.statusCode,400,response.body);assert.match(response.body,/店铺已打烊/);}finally{await app.close();}
});
test('男女各六个基础角色，初次创建重试不重复发钱',async()=>{for(const gender of ['MALE','FEMALE'])assert.equal(initialWorld.appearances.filter(a=>a.partType==='BASE'&&a.genderScope===gender).length,6);const repo=new MemoryRepository(),p=await repo.login('a'),s=new GameService(repo),body=appearance();await Promise.all([s.action(p.id,'create',body),s.action(p.id,'create',body)]);const saved=await repo.player(p.id);assert.equal(saved.cash,120);assert.equal(saved.ledger.length,1);await assert.rejects(()=>s.action(p.id,'create',appearance()));assert.equal((await repo.login('a')).appearance?.baseAvatarId,'FEMALE_01');});
test('完整 HTTP 首登、建角、出门、买入卖出以及账本链路',async()=>{
 ...
 await post('/v1/player/appearance/create',appearance());await completeOpening(post);
 ...
 for(const x of [15,21,27,32.5])await post('/v1/player/move',{x,y:20});
 await post('/v1/world/enter',{plotId:'P_BAISHI_003',entranceId:'ENT_BAISHI_TRADE_S'});await post('/v1/player/move',{x:12,y:14});await post('/v1/npc/talk',{npcId:'NPC_TRADE_CLERK'});
 await post('/v1/player/move',{x:12,y:18});await post('/v1/world/portal',{portalId:'EXIT_B_TRADE'});for(const x of [27,21])await post('/v1/player/move',{x,y:20});
 ...
 await post('/v1/player/move',{x:12,y:18});await post('/v1/world/portal',{portalId:'EXIT_B_GROCERY'});for(const x of [27,32.5])await post('/v1/player/move',{x,y:20});await post('/v1/world/enter',{plotId:'P_BAISHI_003',entranceId:'ENT_BAISHI_TRADE_S'});await post('/v1/economy/sell',{buildingId:'B_TRADE',itemId:'RICE_01',quantity:1});const sold=await post('/v1/quest/finalize',{questId:'Q_001'});assert.equal(sold.player.cash,144);assert.equal(sold.player.inventory.RICE_01,undefined);assert.deepEqual(sold.player.ledger.map((l:any)=>l.after),[120,120,120,120,108,124,124,144]);
 }finally{await app.close();}
});
...
test('外观所有权、性别兼容、Ghost同步',async()=>{const {repo,p,service:s}=await fixture();repo.players.get(p.id)!.sceneId='INTERIOR_B_CLOTH';const change={requestId:randomUUID(),buildingId:'B_CLOTH',appearanceId:'TOP_FEMALE_02',colorId:'ROSE'};await assert.rejects(()=>s.action(p.id,'changeAppearance',change),/尚未拥有/);await s.action(p.id,'purchaseAppearance',{requestId:randomUUID(),buildingId:'B_CLOTH',appearanceId:'TOP_FEMALE_02'});await s.action(p.id,'changeAppearance',change);const saved=await repo.player(p.id);assert.equal(saved.cash,90);assert.equal(saved.appearance?.baseAvatarId,'FEMALE_01');assert.equal((await repo.ghosts('other'))[0].appearance.topStyleId,'TOP_FEMALE_02');assert.equal((await repo.ghosts('other'))[0].appearance.topColorId,'ROSE');await assert.rejects(()=>s.action(p.id,'purchaseAppearance',{requestId:randomUUID(),buildingId:'B_CLOTH',appearanceId:'TOP_MALE_02'}));});
...
test('REPORT 配置必须绑定目标 NPC 且目标保持一致',()=>{const bad=structuredClone(initialWorld);const report=bad.quests.find(q=>q.id==='Q_003')!.steps[2];report.target='NPC_SALON_HAIRDRESSER';assert.throws(()=>validateWorld(bad),/汇报步骤/);});
...
test('第一桶金任务接口返回统一配置、步骤进度和奖励状态',async()=>{const repo=new MemoryRepository(),app=await buildApp(repo,env);try{const auth=(await app.inject({method:'POST',url:'/v1/auth/dev',payload:{account:'quest-test'}})).json(),headers={authorization:`Bearer ` + auth.token};await app.inject({method:'POST',url:'/v1/player/appearance/create',headers,payload:{requestId:randomUUID(),gender:'FEMALE',faceId:'F_FACE_01',profile:testProfile()}});Object.assign(repo.players.get(auth.player.id)!,{storyFlags:{FIRST_DAY_ENTERED_BAISHI:true},sceneId:'INTERIOR_B_TRADE'});const before=(await app.inject({url:'/v1/quests',headers})).json();assert.equal(before.quests[0].state,'available');assert.equal(before.quests[0].steps[0].title,'购买鸣山大米');assert.match(before.quests[0].steps[0].objective,/街坊杂货铺/);assert.deepEqual(before.quests[0].stepProgress,[0,0]);assert.equal(before.quests[0].reward,20);}finally{await app.close();}});
...
test('云存档验收 A/B：退出重登恢复状态且任务奖励与出售不可重复',async()=>{
 const repo=new MemoryRepository(),app=await buildApp(repo,env);try{
  ...
  await completeOpening((url,payload)=>post(first.headers,url,payload));
  ...
  await post(first.headers,'/v1/player/move',{x:12,y:18});await post(first.headers,'/v1/world/portal',{portalId:'EXIT_B_TRADE'});
  ...
  await post(againA.headers,'/v1/player/move',{x:12,y:18});await post(againA.headers,'/v1/world/portal',{portalId:'EXIT_B_GROCERY'});for(const x of [27,32.5])await post(againA.headers,'/v1/player/move',{x,y:20});await post(againA.headers,'/v1/world/enter',{plotId:'P_BAISHI_003',entranceId:'ENT_BAISHI_TRADE_S'});
  const sell={requestId:randomUUID(),buildingId:'B_TRADE',itemId:'RICE_01',quantity:1};await post(againA.headers,'/v1/economy/sell',sell,sell.requestId);const sold=await post(againA.headers,'/v1/quest/finalize',{questId:'Q_001'});assert.equal(sold.player.cash,144);assert.equal(sold.player.inventory.RICE_01,undefined);assert.equal(sold.player.ledger.filter((l:any)=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_001').length,1);
...
test('白石街 NPC 首次交谈、重复交谈与重新登录记录',async()=>{const repo=new MemoryRepository(),p=await repo.login('npc-loop'),service=new GameService(repo);await service.action(p.id,'create',appearance());const saved=repo.players.get(p.id)!;saved.sceneId='STREET_BAISHI_01';saved.x=16;saved.y=22.5;const first=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_002'});assert.match(first.dialogue,/白石街/);assert.deepEqual((await repo.player(p.id)).metNpcs,['NPC_001','NPC_002']);const second=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_002'});assert.match(second.dialogue,/白石街/);assert.deepEqual((await repo.login('npc-loop')).metNpcs,['NPC_001','NPC_002']);});

test('五名白石街核心 NPC 共用 metNpcs 并持久化唯一认识记录',async()=>{const repo=new MemoryRepository(),p=await repo.login('core-npc-relations'),service=new GameService(repo);await service.action(p.id,'create',appearance());repo.players.get(p.id)!.storyFlags={INTRO_INN_KEEPER_DONE:true};const core=[['NPC_001','INTERIOR_B_INN',8,9],['NPC_TRADE_CLERK','INTERIOR_B_TRADE',12,9],['NPC_GROCERY_CLERK','INTERIOR_B_GROCERY',12,9],['NPC_SALON_HAIRDRESSER','INTERIOR_B_SALON',8,9],['NPC_CLOTH_SHOPKEEPER','INTERIOR_B_CLOTH',16,9]] as const;for(const [npcId,sceneId,x,y] of core){const saved=repo.players.get(p.id)!;saved.sceneId=sceneId;saved.x=x;saved.y=y;await service.action(p.id,'talk',{requestId:randomUUID(),npcId});await service.action(p.id,'talk',{requestId:randomUUID(),npcId});}assert.deepEqual((await repo.login('core-npc-relations')).metNpcs,core.map(([npcId])=>npcId));});

test('陈掌柜开场接取急差，第一桶金由商行伙计接取且均防重复',async()=>{const repo=new MemoryRepository(),p=await repo.login('quest-accept'),service=new GameService(repo);await service.action(p.id,'create',appearance());const saved=repo.players.get(p.id)!;saved.sceneId='INTERIOR_B_INN';saved.x=12;saved.y=18;const accepted=await service.action(p.id,'introComplete',{requestId:randomUUID()});assert.equal(accepted.guide,'接到任务\n《掌柜的急差》');assert.equal((await repo.player(p.id)).ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_002'),true);const firstExit=await service.action(p.id,'portal',{requestId:randomUUID(),portalId:'EXIT_B_INN'});assert.equal(firstExit.guide,undefined);assert.equal((await repo.player(p.id)).ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_001'),false);Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_TRADE',x:12,y:9});await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_TRADE_CLERK'});assert.equal((await repo.player(p.id)).ledger.filter(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_001').length,1);});

test('掌柜的急差流程支持恢复且最终对白后才结算一次',async()=>{const repo=new MemoryRepository(),p=await repo.login('q002-flow'),service=new GameService(repo);await service.action(p.id,'create',appearance());repo.players.get(p.id)!.sceneId='INTERIOR_B_INN';repo.players.get(p.id)!.storyFlags={INTRO_INN_KEEPER_DONE:true,FIRST_DAY_RETURNED_TO_INN:true};repo.players.get(p.id)!.x=8;repo.players.get(p.id)!.y=9;const accepted=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_001'});assert.match(accepted.dialogue,/急件/);repo.players.get(p.id)!.sceneId='INTERIOR_B_GROCERY';repo.players.get(p.id)!.x=12;repo.players.get(p.id)!.y=9;await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_GROCERY_CLERK'});assert.equal((await repo.player(p.id)).inventory.ERRAND_PACKAGE_01,1);const restored=await repo.login('q002-flow');assert.equal(restored.inventory.ERRAND_PACKAGE_01,1);repo.players.get(p.id)!.sceneId='INTERIOR_B_INN';repo.players.get(p.id)!.x=8;const ready:any=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_001'});assert.equal(ready.readyToFinalizeQuestId,'Q_002');assert.equal((await repo.player(p.id)).inventory.ERRAND_PACKAGE_01,1);await service.action(p.id,'finalizeQuest',{requestId:randomUUID(),questId:'Q_002'});const completed=await repo.player(p.id);assert.equal(completed.inventory.ERRAND_PACKAGE_01,undefined);assert.equal(completed.ledger.filter(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_002').length,1);await assert.rejects(()=>service.action(p.id,'finalizeQuest',{requestId:randomUUID(),questId:'Q_002'}));});

test('雨前送样按 ACQUIRE、DELIVER、REPORT 顺序推进，并在最终对白后发奖',async()=>{const repo=new MemoryRepository(),p=await repo.login('rain-sample-flow'),service=new GameService(repo);await service.action(p.id,'create',appearance());repo.players.get(p.id)!.storyFlags={FIRST_DAY_COMPLETE:true};const place=(sceneId:string,x:number,y:number)=>Object.assign(repo.players.get(p.id)!,{sceneId,x,y});place('INTERIOR_B_CLOTH',16,9);const acquired=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});assert.match(acquired.dialogue,/新料子/);let saved=await repo.player(p.id);assert.equal(saved.inventory.CLOTH_SAMPLE_01,1);place('INTERIOR_B_SALON',8,9);const delivered=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_SALON_HAIRDRESSER'});assert.match(delivered.dialogue,/再柔一点/);saved=await repo.player(p.id);assert.equal(saved.inventory.CLOTH_SAMPLE_01,undefined);assert.equal(saved.ledger.some(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003'),false);place('INTERIOR_B_CLOTH',16,9);const reported:any=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});assert.equal(reported.readyToFinalizeQuestId,'Q_003');saved=await repo.player(p.id);assert.equal(saved.cash,120);await service.action(p.id,'finalizeQuest',{requestId:randomUUID(),questId:'Q_003'});saved=await repo.player(p.id);assert.equal(saved.cash,135);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003').length,1);});

test('雨前送样交付后重登恢复 REPORT 步骤并可完成',async()=>{const repo=new MemoryRepository(),p=await repo.login('rain-sample-relogin'),service=new GameService(repo);await service.action(p.id,'create',appearance());repo.players.get(p.id)!.storyFlags={FIRST_DAY_COMPLETE:true};Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_CLOTH',x:16,y:9});await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_SALON',x:8,y:9});await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_SALON_HAIRDRESSER'});let restored=await repo.login('rain-sample-relogin');assert.equal(restored.inventory.CLOTH_SAMPLE_01,undefined);assert.equal(restored.ledger.some(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003'),false);Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_CLOTH',x:16,y:9});const ready:any=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});assert.equal(ready.readyToFinalizeQuestId,'Q_003');await service.action(p.id,'finalizeQuest',{requestId:randomUUID(),questId:'Q_003'});restored=await repo.login('rain-sample-relogin');assert.equal(restored.ledger.filter(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003').length,1);});
