...
test('陈掌柜开场接取急差，第一桶金在出客栈后接取且均防重复',async()=>{const repo=new MemoryRepository(),p=await repo.login('quest-accept'),service=new GameService(repo);await service.action(p.id,'create',appearance());const saved=repo.players.get(p.id)!;saved.sceneId='INTERIOR_B_INN';saved.x=12;saved.y=18;const accepted=await service.action(p.id,'introComplete',{requestId:randomUUID()});assert.equal(accepted.guide,'接到任务\n《掌柜的急差》');assert.equal((await repo.player(p.id)).ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_002'),true);const firstExit=await service.action(p.id,'portal',{requestId:randomUUID(),portalId:'EXIT_B_INN'});assert.equal(firstExit.guide,'接到任务\n《第一桶金》');assert.equal((await repo.player(p.id)).ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_001'),true);Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_INN',x:8,y:9});await assert.rejects(()=>service.action(p.id,'acceptQuest',{requestId:randomUUID(),questId:'Q_001'}),/任务已经接取/);});
