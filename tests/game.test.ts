...
test('雨前送样按 ACQUIRE、DELIVER、REPORT 顺序推进并只在 REPORT 发奖',async()=>{const repo=new MemoryRepository(),p=await repo.login('rain-sample-flow'),service=new GameService(repo);await service.action(p.id,'create',appearance());repo.players.get(p.id)!.storyFlags={FIRST_DAY_COMPLETE:true};const place=(sceneId:string,x:number,y:number)=>Object.assign(repo.players.get(p.id)!,{sceneId,x,y});place('INTERIOR_B_CLOTH',16,9);const acquireRequest={requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'};const acquired=await service.action(p.id,'talk',acquireRequest);assert.match(acquired.dialogue,/这批新布刚到/);let saved=await repo.player(p.id);assert.equal(saved.inventory.CLOTH_SAMPLE_01,1);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_003').length,1);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_ITEM_ACQUIRED'&&l.referenceId==='Q_003:CLOTH_SAMPLE_01').length,1);assert.ok(saved.metNpcs.includes('NPC_CLOTH_SHOPKEEPER'));await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});saved=await repo.player(p.id);assert.equal(saved.inventory.CLOTH_SAMPLE_01,1);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_003').length,1);assert.equal(saved.ledger.some(l=>l.type==='QUEST_REPORTED'&&l.referenceId.startsWith('Q_003:')),false,'未交付前不能提前 REPORT');place('INTERIOR_B_SALON',8,9);const delivered=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_SALON_HAIRDRESSER'});assert.match(delivered.dialogue,/再压一点青/);saved=await repo.player(p.id);assert.equal(saved.inventory.CLOTH_SAMPLE_01,undefined);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_ITEM_DELIVERED'&&l.referenceId==='Q_003:CLOTH_SAMPLE_01').length,1);assert.equal(saved.ledger.some(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003'),false,'DELIVER 后任务不能完成');assert.ok(saved.metNpcs.includes('NPC_SALON_HAIRDRESSER'));await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_SALON_HAIRDRESSER'});saved=await repo.player(p.id);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_ITEM_DELIVERED'&&l.referenceId==='Q_003:CLOTH_SAMPLE_01').length,1);assert.equal(saved.ledger.some(l=>l.type==='QUEST_REPORTED'&&l.referenceId.startsWith('Q_003:')),false,'错误 NPC 无法 REPORT');place('INTERIOR_B_CLOTH',16,9);const reported=await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});assert.match(reported.dialogue,/青里再压一点/);assert.match(reported.dialogue,/任务完成：雨前送样/);saved=await repo.player(p.id);assert.equal(saved.cash,135);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_REPORTED'&&l.referenceId==='Q_003:NPC_CLOTH_SHOPKEEPER').length,1);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003').length,1);await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});saved=await repo.player(p.id);assert.equal(saved.cash,135);assert.equal(saved.ledger.filter(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003').length,1);});

test('雨前送样交付后重登恢复 REPORT 步骤并可完成',async()=>{const repo=new MemoryRepository(),p=await repo.login('rain-sample-relogin'),service=new GameService(repo);await service.action(p.id,'create',appearance());repo.players.get(p.id)!.storyFlags={FIRST_DAY_COMPLETE:true};Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_CLOTH',x:16,y:9});await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_SALON',x:8,y:9});await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_SALON_HAIRDRESSER'});let restored=await repo.login('rain-sample-relogin');assert.equal(restored.inventory.CLOTH_SAMPLE_01,undefined);assert.equal(restored.ledger.some(l=>l.type==='QUEST_ITEM_DELIVERED'&&l.referenceId==='Q_003:CLOTH_SAMPLE_01'),true);assert.equal(restored.ledger.some(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003'),false);Object.assign(repo.players.get(p.id)!,{sceneId:'INTERIOR_B_CLOTH',x:16,y:9});await service.action(p.id,'talk',{requestId:randomUUID(),npcId:'NPC_CLOTH_SHOPKEEPER'});restored=await repo.login('rain-sample-relogin');assert.equal(restored.ledger.filter(l=>l.type==='QUEST_REWARD'&&l.referenceId==='Q_003').length,1);});
