...
test('第一桶金接取状态可保存并防止重复接取',async()=>{const repo=new MemoryRepository(),p=await repo.login('quest-accept'),service=new GameService(repo);await service.action(p.id,'create',appearance());const saved=repo.players.get(p.id)!;saved.sceneId='INTERIOR_B_INN';saved.storyFlags={INTRO_INN_KEEPER_DONE:true,FIRST_DAY_ENTERED_BAISHI:true};saved.x=8;saved.y=9;const accepted=await service.action(p.id,'acceptQuest',{requestId:randomUUID(),questId:'Q_001'});assert.match(accepted.dialogue,/已接取任务/);assert.equal((await repo.player(p.id)).ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId==='Q_001'),true);await assert.rejects(()=>service.action(p.id,'acceptQuest',{requestId:randomUUID(),questId:'Q_001'}),/任务已经接取/);});
