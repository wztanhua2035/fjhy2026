...
- 春衫衣坊正式美术接入硬约束：保持西侧入口 `ENT_BAISHI_CLOTH_W`，视觉可进入门洞必须与 `(26.5,33)` 的 west 向入口及 `2.5×2.6` trigger 基本对应；不得制作仅南侧正门可进入、却保留西侧逻辑入口的冲突建筑。室内出口固定为 `(12,18)`。
## 主角模块化外观系统（设计预留）

当前男女主角行走 spritesheet 与 portrait 定义为默认外观和 fallback。未来主角可通过美发室、衣坊等系统更换发型、服装与饰品，但不采用“每种发型 × 每套衣服”重新制作完整角色的组合方式，而采用 `基础身体 + 发型 + 服装 + 饰品` 的模块化方案。

后续行走角色各层统一为 `256×256`、4×4、单帧 `64×64`，使用相同 directionRows、footAnchor 与身体/头部/肩部基准。portrait 采用半模块化分层：`后发 → 身体/脸 → 衣服 → 前发 → 饰品`。所有外观层必须等比制作和对齐，禁止拉伸。

玩家换装时优先由客户端合成并缓存当前 spritesheet / portrait，不要求服务端保存合成图片。服务端未来只保存 `hairId`、`outfitId`、`accessoryId` 与已拥有 / 已解锁外观；“拥有外观”与“当前穿戴外观”必须区分。
