...
type HairServiceOptions={contextButton?:boolean;webLife?:boolean};
...
export function installHairService(scene:Phaser.Scene,game:GameController,sprite:Phaser.GameObjects.Sprite,width:number,height:number,creation?:()=>{gender:'MALE'|'FEMALE';faceId:string;hairId:string},options:HairServiceOptions={}){
  ...
  const info=text(width/2+70,height/2-48,'').setWordWrapWidth(330);
  ...
  const previous=button(width/2-25,height/2+20,'上一款',()=>game.cycleHair(-1));
  const next=button(width/2+170,height/2+20,'下一款',()=>game.cycleHair(1));
  const confirm=button(width/2+120,height/2+90,'确认更换',()=>run(()=>game.confirmHair()),'primary');
  const cancel=button(width/2-120,height/2+90,'返回',()=>game.closeHairService(),'ghost');
  ...
  const update=()=>{
    ...
    const visible=game.hairPanelOpen&&!options.webLife;
    for(const object of panel)object.setVisible(visible);
    if(!visible)return;
    const current=game.hairPanel(),cash=game.player?.cash??0,insufficient=!!current&&cash<current.price;
