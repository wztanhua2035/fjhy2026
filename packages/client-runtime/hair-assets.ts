...
const root='/scene-layers/baishi/formal/face-v1';
export const hairAssets=[
  ...(['male','female'] as const).map(g=>({key:`player-${g}-body-v4`,source:`${root}/player_${g}_body_v4.png`})),
  ...faceConfigs.map(f=>({key:f.assetResourceId,source:`${root}/${f.faceId.toLowerCase()}_v1.png`})),
