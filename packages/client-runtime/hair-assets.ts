...
const root='/scene-layers/baishi/formal/hair-v2';
export const hairAssets=[
  ...(['male','female'] as const).map(g=>({key:`player-${g}-body-v2`,source:`${root}/player_${g}_body_v2.png`})),
  ...hairConfigs.map(h=>({key:h.assetResourceId,source:`${root}/${h.hairId.toLowerCase()}_v2.png`}))
];
