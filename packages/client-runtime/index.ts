...
import { GUEST_ROOM_SCENE_ID, INN_LOBBY_SCENE_ID, INTRO_INN_KEEPER_DONE, guestRoomOpeningDialogue, innOpeningDialogue } from '../game-config/inn-opening.js';
...
export class GameController {
  token='';boot:Bootstrap|null=null;view:SceneView|null=null;ghosts:GhostProfile[]=[];quests:QuestRuntime[]=[];direction:Direction='down';walkTime=0;moving=false;private interactionCooldown=0;private introIndex=-1;private guestIntroIndex=-1;
    x=12;y=15;busy=false;offline=false;message='欢迎来到横阳';guide:string|null=null;dialogue:string|null=null;dialogueSpeaker:string|null=null;pending:{path:string;body:any}|null=null;private dialogueQueue:{speakerId:string;speaker:string;text:string}[]=[];private pendingQuestFinalization:string|null=null;private interactionLabel='';private activeInteraction:InteractionCandidate|null=null;private lastSync=0;private syncInFlight:Promise<unknown>|null=null;private correctionX=0;private correctionY=0;
  private movePath:{x:number;y:number}[]=[];
  private interacting=false;
  get actionPending(){return this.interacting||this.busy||!!this.pending;}
  transitionId=0;transitionPending=false;transitionStartedAt=0;
  private sceneLoadSequence=0;
  private beginTransition(){this.transitionId++;this.transitionPending=true;this.transitionStartedAt=Date.now();return this.transitionId;}
  ...
  facilityItems:{id:string;name:string;description?:string;questItem?:boolean;keyItem?:boolean;questOnly?:boolean}[]=[];
  ...
  async restart(){this.closeHairService();this.closeOutfitShop();this.closeGuestFacility();await this.write('/v1/player/restart',{confirm:true});this.view=null;this.ghosts=[];this.quests=[];this.dialogue=null;this.dialogueSpeaker=null;this.dialogueQueue=[];this.pendingQuestFinalization=null;this.introIndex=-1;this.guestIntroIndex=-1;await this.refresh();}
  ...
  private showGuestIntroLine(){const line=guestRoomOpeningDialogue[this.guestIntroIndex];this.dialogue=line.text;this.dialogueSpeaker=line.speaker;this.message=line.text;this.onChange();}
  private showQueuedDialogue(){const line=this.dialogueQueue.shift();if(!line)return false;this.dialogue=line.text;this.dialogueSpeaker=line.speaker;this.message=line.text;this.onChange();return true;}
  async advanceDialogue(){if(this.guestIntroIndex>=0){if(this.guestIntroIndex<guestRoomOpeningDialogue.length-1){this.guestIntroIndex++;this.showGuestIntroLine();return;}this.guestIntroIndex=-1;}else if(this.introIndex>=0){if(this.introIndex<innOpeningDialogue.length-1){this.introIndex++;this.showIntroLine();return;}await this.write('/v1/intro/complete',{});this.introIndex=-1;if(this.dialogue)return;}if(this.showQueuedDialogue())return;if(this.pendingQuestFinalization){const questId=this.pendingQuestFinalization;this.pendingQuestFinalization=null;await this.write('/v1/quest/finalize',{questId});}this.dialogue=null;this.dialogueSpeaker=null;if(!this.message.startsWith('任务完成'))this.message='';this.onChange();}
  async loadScene(transitionToken?:number){
    const sequence=++this.sceneLoadSequence,sceneId=this.player!.sceneId;
    const transition=transitionToken??this.beginTransition();
    this.closeHairService();this.closeOutfitShop();this.closeGuestFacility();this.shopOpen=false;this.movePath=[];this.correctionX=0;this.correctionY=0;
    try{
      const view=await this.transport(`/v1/world/scenes/${sceneId}`,undefined,this.token);
      if(sequence!==this.sceneLoadSequence||transition!==this.transitionId||this.player?.sceneId!==sceneId)return;
      this.view=view;const position=view.playerPosition;
      if(position&&position.sceneId===sceneId){this.boot!.player.x=position.x;this.boot!.player.y=position.y;this.x=position.x;this.y=position.y;}
      this.ghosts=[];this.transitionPending=false;this.onChange();
      this.transport(`/v1/scenes/${sceneId}/ghosts`,undefined,this.token).then(data=>{if(sequence!==this.sceneLoadSequence||this.player?.sceneId!==sceneId)return;this.ghosts=data.ghosts;this.onChange();}).catch(()=>{});
    }finally{if(sequence===this.sceneLoadSequence&&transition===this.transitionId){this.transitionPending=false;this.onChange();}}
  }
...
try{const knownNpcs=new Set(this.player?.metNpcs??[]),npc=this.view?.npcs.find(n=>n.id===op.body?.npcId);const result=await this.transport(op.path,op.body,this.token);this.pending=null;this.offline=false;this.guide=typeof result.guide==='string'?result.guide:null;const lines=Array.isArray(result.dialogueLines)?result.dialogueLines:[];this.dialogueQueue=lines.slice(1);this.dialogue=lines[0]?.text??(typeof result.dialogue==='string'?result.dialogue:null);this.dialogueSpeaker=this.dialogue?(lines[0]?.speaker??result.speaker??npc?.name??'白石街'):null;this.pendingQuestFinalization=typeof result.readyToFinalizeQuestId==='string'?result.readyToFinalizeQuestId:null;if(result.player){const old=this.player?.sceneId;this.boot!.player=result.player;this.x=result.player.x;this.y=result.player.y;if(op.path==='/v1/player/restart'){this.view=null;this.ghosts=[];this.quests=[];this.guide=null;}else if(old!==result.player.sceneId||!this.view)await this.loadScene(this.transitionPending?this.transitionId:undefined);if(['/v1/world/portal','/v1/world/enter','/v1/intro/complete','/v1/quest/finalize','/v1/npc/talk','/v1/economy/buy','/v1/economy/sell'].includes(op.path))try{this.quests=(await this.transport('/v1/quests',undefined,this.token)).quests??this.quests;}catch{/* The action already committed; quest list refresh can wait for reconnect. */}}const relationship=op.path==='/v1/npc/talk'&&npc?(knownNpcs.has(npc.id)?'关系状态：已认识。\n':'初次结识：'+npc.name+'。\n关系状态：已认识。\n'):'';this.message=relationship+(this.dialogue??result.dialogue??'操作已完成');if(op.path==='/v1/player/appearance/create'){this.guestIntroIndex=0;this.showGuestIntroLine();}this.startIntroIfNeeded();return result;}
  ...
  questTracker():QuestTrackerItem[]{const ledger=this.player?.ledger??[];const tasks:QuestTrackerItem[]=this.quests.flatMap(quest=>{const accepted=ledger.some(entry=>entry.type==='QUEST_ACCEPTED'&&entry.referenceId===quest.id),completed=ledger.some(entry=>entry.type==='QUEST_REWARD'&&entry.referenceId===quest.id);if(!accepted&&!completed)return [];const progress=questStepProgress(quest,ledger),pendingIndex=progress.findIndex((value,index)=>value<quest.steps[index].count),stepIndex=completed?quest.steps.length:pendingIndex<0?quest.steps.length-1:pendingIndex,step=quest.steps[Math.min(stepIndex,quest.steps.length-1)],awaitingReward=!completed&&pendingIndex<0;const fallbackTitle=`${step?.type??'任务'} ${step?.target??''}`.trim(),fallbackObjective=step?`${fallbackTitle} ×${step.count}`:'任务已完成';return [{id:quest.id,name:quest.name,state:completed?'completed':progress.some(Boolean)?'in_progress':'accepted',stepIndex,stepCount:quest.steps.length,currentStep:completed?'全部步骤已完成':awaitingReward?'全部步骤已完成 · 等待结算':`步骤 ${stepIndex+1}/${quest.steps.length} · ${step?.title??fallbackTitle}`,currentObjective:completed?'任务已完成':awaitingReward?'等待任务奖励结算':step?.objective??fallbackObjective,rewardSummary:`奖励：${quest.reward} 文`,completed}];});return tasks.sort((a,b)=>Number(a.completed)-Number(b.completed)||Number(b.id==='Q_002')-Number(a.id==='Q_002')||ledger.findIndex(entry=>entry.type==='QUEST_ACCEPTED'&&entry.referenceId===a.id)-ledger.findIndex(entry=>entry.type==='QUEST_ACCEPTED'&&entry.referenceId===b.id));}
  ...
  async trade(action:'buy'|'sell',itemId:string,quantity=1){
    ...
    try {
      ...
      if(!this.dialogue)this.message=[action==='buy'?'购买成功':'出售成功',`${this.itemName(itemId)} ×${actual?.quantity??quantity}`,`${action==='buy'?'支出':'收入'}：${actual?.total??Math.abs(entry?.amount??0)}文`].join('\n');
