...
def frame_base(original,row):
    ...
    assert base.crop((0,32,64,64)).tobytes()==original.crop((0,32,64,64)).tobytes()
    return base,cx
...
def build(gender,atlas_path):
    ...
    for row in range(4):
        ...
        for col in range(4):
            ...
            bald,cx=frame_base(frame,row);base.paste(bald,(col*64,row*64))
            for style,hair in enumerate(hairs):
                width=(29 if gender=='male' else 31)+(2 if style==2 else 0)
                height=round(hair.height*width/hair.width)
                if height>37:width=round(width*37/height);height=37
                small=hair.resize((width,height),Image.Resampling.LANCZOS)
                x=round(cx-width/2);y=2 if gender=='female' else 4
    ...
    paths=[]
    for name,img in [(f'player_{gender}_body_v1',base),*[(f'{"m" if gender=="male" else "f"}_hair_0{i+1}_v1',img) for i,img in enumerate(layers)]]:
