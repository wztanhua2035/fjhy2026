...
export async function ensureGroceryShop(repo:Repository){
  ...
  const inventoryV11Ready=previous.items.every(item=>item.basePrice===undefined&&(!item.usable||!!item.effectType))&&previous.buildings.every(building=>Object.values(building.stock).every(offer=>(offer.canBuy===false||offer.baseBuyPrice!==undefined)&&(offer.canSell===false||offer.baseSellPrice!==undefined)&&!!offer.pricingMode&&!!offer.stockMode&&offer.stockMode!=='infinite'));
