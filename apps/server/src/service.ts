  ...
  async action(playerId:string,action:string,body:any){
    ...
    return this.repo.mutate(playerId,body.requestId,hash,p=>{
      ...
      switch(action){
        ...
        case 'buy':case 'sell':{
          const shop=this.shop(world,p,body.buildingId),stock=shop.stock[body.itemId],item=world.items.find(i=>i.id===body.itemId);ensure(stock&&item,'NOT_SOLD','该店不经营此商品');ensure(!item.questOnly,'QUEST_ITEM','任务物品不可买卖');
        ...
        case 'acceptQuest':{
          ...
          const met=p.metNpcs.includes(npc.id);if(!met)p.metNpcs.push(npc.id);let dialogue=npc.dialogue[met?Math.min(1,npc.dialogue.length-1):0];
          for(const quest of world.quests.filter(q=>q.enabled&&p.ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId===q.id)&&!p.ledger.some(l=>l.type==='QUEST_REWARD'&&l.referenceId===q.id))){const progress=quest.steps.map(step=>{const type=step.type==='BUY'?'SHOP_BUY':step.type==='SELL'?'SHOP_SELL':step.type==='ACQUIRE'?'QUEST_ITEM_ACQUIRED':step.type==='DELIVER'?'QUEST_ITEM_DELIVERED':'';const reference=step.type==='ACQUIRE'||step.type==='DELIVER'?`${quest.id}:${step.target}`:step.target;return type?p.ledger.filter(l=>l.type===type&&l.referenceId===reference).length:0;});const index=progress.findIndex((value,i)=>value<quest.steps[i].count),step=quest.steps[index];if(!step||step.npcId!==npc.id)continue;const item=world.items.find(i=>i.id===step.target);ensure(item,'QUEST_ITEM','任务物品不存在');if(step.type==='ACQUIRE'){if((p.inventory[item.id]??0)>=step.count||progress[index]>=step.count){dialogue='已领取：'+item.name+'。';continue;}p.inventory[item.id]=(p.inventory[item.id]??0)+step.count;p.ledger.push({id:randomUUID(),type:'QUEST_ITEM_ACQUIRED',amount:0,before:p.cash,after:p.cash,referenceId:`${quest.id}:${item.id}`,requestId:body.requestId,createdAt:new Date().toISOString()});dialogue='已领取：'+item.name+'。请带回横阳客栈交给陈掌柜。';}else if(step.type==='DELIVER'){ensure((p.inventory[item.id]??0)>=step.count,'QUEST_ITEM_MISSING','请先领取任务物品');p.inventory[item.id]-=step.count;if(p.inventory[item.id]===0)delete p.inventory[item.id];p.ledger.push({id:randomUUID(),type:'QUEST_ITEM_DELIVERED',amount:0,before:p.cash,after:p.cash,referenceId:`${quest.id}:${item.id}`,requestId:body.requestId,createdAt:new Date().toISOString()});money(p,quest.reward,'QUEST_REWARD',quest.id,body.requestId);dialogue='任务完成：'+quest.name+'，获得 '+quest.reward+' 文奖励';}}
          if(npc.questId&&!p.ledger.some(l=>l.type==='QUEST_ACCEPTED'&&l.referenceId===npc.questId)){const quest=world.quests.find(q=>q.id===npc.questId&&q.enabled);if(quest){p.ledger.push({id:randomUUID(),type:'QUEST_ACCEPTED',amount:0,before:p.cash,after:p.cash,referenceId:quest.id,requestId:body.requestId,createdAt:new Date().toISOString()});dialogue='任务已接取\n任务：'+quest.name+'\n当前目标：'+(quest.steps[0]?.objective??'继续任务。')+'\n'+dialogue;}}
