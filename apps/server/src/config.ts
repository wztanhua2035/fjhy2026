...
export const worldSchema=z.object({
  ...
  npcs:z.array(z.object({id,name,nameLocked:z.boolean(),enabled:z.boolean(),sceneId:id,x:z.number(),y:z.number(),priority:z.number().int().min(0).max(100),hours,dialogue:z.array(z.string().min(1).max(200)).min(1).max(10),route:z.array(point).max(20),questId:id.optional(),appearance:z.object({gender:z.enum(["MALE","FEMALE"]),baseAvatarId:id,skinColorId:id.optional(),hairStyleId:id,hairColorId:id,topStyleId:id,topColorId:id,bottomStyleId:id,bottomColorId:id,shoesId:id,accessoryIds:z.array(id)}).optional()})).max(200),
  items:z.array(z.object({id,name,icon:z.string().min(1).max(8).optional(),basePrice:z.number().int().min(1).max(1e6),giftable:z.boolean(),stackMax:z.number().int().min(1).max(999)})).max(200),
  ...
  quests:z.array(z.object({id,name,steps:z.array(z.object({type:id,target:id,count:z.number().int().positive(),title:z.string().min(1).max(60).optional(),objective:z.string().min(1).max(160).optional()})),reward:z.number().int().min(0).max(1e6),enabled:z.boolean()})).max(100),
