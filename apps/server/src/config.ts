...
export const worldSchema=z.object({
  ...
  scenes:z.array(z.object({id,name,townId:id,width:z.number().int().min(20).max(160),height:z.number().int().min(20).max(120),tileSize:z.literal(32),mapAsset:z.string().regex(/^maps\/[a-z0-9_-]+\.tmx$/),roads:z.array(rect),collision:z.array(rect),spawnX:z.number(),spawnY:z.number(),buildingId:id.optional(),portals:z.array(z.object({id,x:z.number(),y:z.number(),toSceneId:id,spawnX:z.number(),spawnY:z.number()}))})).min(1).max(100),
  plots:z.array(rect.extend({id,townId:id,districtId:id,sceneId:id,plotType:id,facing:id,entranceX:z.number(),entranceY:z.number(),status:z.enum(['EMPTY','RESERVED','NPC_OCCUPIED','PLAYER_OWNED','MUNICIPAL','EVENT','CONSTRUCTION','LOCKED']),allowedBuildingTypes:z.array(id),buildingId:id.nullable(),version:z.number().int().positive()})).max(200),
  buildings:z.array(z.object({id,name,buildingType:id,assetKey:z.string().min(1).max(150),interiorSceneId:id,openingHours:hours,enabled:z.boolean(),buyable:z.boolean(),baseValue:z.number().int().min(0).max(1e9),stock:z.record(id,z.object({buy:z.number().int().min(1).max(1e6),sell:z.number().int().min(1).max(1e6),dailyLimit:z.number().int().min(1).max(1000)}))})).max(200),
  npcs:z.array(z.object({id,name,nameLocked:z.boolean(),enabled:z.boolean(),sceneId:id,x:z.number(),y:z.number(),priority:z.number().int().min(0).max(100),hours,dialogue:z.array(z.string().min(1).max(200)).min(1).max(10),route:z.array(point).max(20)})).max(200),
  items:z.array(z.object({id,name,basePrice:z.number().int().min(1).max(1e6),giftable:z.boolean(),stackMax:z.number().int().min(1).max(999)})).max(200),
  appearances:z.array(z.object({id,partType:z.enum(['BASE','HAIR','TOP','BOTTOM','SHOES','ACCESSORY']),name,genderScope:z.enum(['MALE','FEMALE','ALL']),assetKey:z.string().min(1).max(150),price:z.number().int().min(0).max(1e6),colors:z.array(id),enabled:z.boolean(),starter:z.boolean()})).max(500),
  quests:z.array(z.object({id,name,steps:z.array(z.object({type:id,target:id,count:z.number().int().positive()})),reward:z.number().int().min(0).max(1e6),enabled:z.boolean()})).max(100),
