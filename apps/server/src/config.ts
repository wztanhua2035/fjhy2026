...
const interiorZone=rect.extend({id,kind:z.enum(['wall','counter','shelf','storage','stairs','room','waitingArea','servicePoint','displayArea','chair','mirror','exit','entry','future']),solid:z.boolean(),label:z.string().max(40).optional()});
export const worldSchema=z.object({
  ...
  scenes:z.array(z.object({id,name,townId:id,width:z.number().int().min(20).max(160),height:z.number().int().min(20).max(120),tileSize:z.literal(32),mapAsset:z.string().regex(/^maps\/[a-z0-9_-]+\.tmx$/),roads:z.array(rect),collision:z.array(rect),spawnX:z.number(),spawnY:z.number(),buildingId:id.optional(),interior:z.object({zones:z.array(interiorZone)}).optional(),portals:z.array(z.object({id,x:z.number(),y:z.number(),toSceneId:id,spawnX:z.number(),spawnY:z.number(),returnEntranceId:id.optional()}))})).min(1).max(100),
