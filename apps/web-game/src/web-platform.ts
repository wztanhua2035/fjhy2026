...
export function createWebPlatform(apiBase = '', debugOpenAll = false): PlatformAdapter {
  const base = apiBase.replace(/\/$/, '');
  const transport: PlatformTransport = async (path, body, token) => {
    const response = await fetch(`${base}${path}`, { method: body ? 'POST' : 'GET', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(import.meta.env.DEV&&debugOpenAll ? { 'X-Debug-Open-All': '1' } : {}) }, body: body ? JSON.stringify(body) : undefined, signal: AbortSignal.timeout(10000) });
