...
import {createHairUi} from './hair-ui';
import {WebFeedbackQueue,formatRpgEventNotice,isTransientSceneMessage,transitionLoading} from './scene-feedback';
...
const refreshHair=createHairUi(controller,hud);
...
function renderPreview(){
  ...
  const size=148;for(const img of images)ctx.drawImage(img,0,row*64,64,64,(canvas.width-size)/2,10,size,size);
}
