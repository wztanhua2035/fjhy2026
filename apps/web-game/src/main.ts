...
import { uiTokens, UiNotificationQueue } from '../../../packages/client-runtime/ui-design-tokens.js';
...
const dialogueUi=createWebDialogueUi(hud,()=>void run(()=>controller.advanceDialogue()));
const notices=new UiNotificationQueue();
let lastNoticeSource='',shownNoticeId=0,noticeTimer:ReturnType<typeof setTimeout>|undefined;
function presentNotice(){const notice=notices.current();message.classList.toggle('hidden',!notice||!!controller.dialogue);if(!notice){shownNoticeId=0;return;}message.textContent=notice.text;message.dataset.kind=notice.kind;message.dataset.event=/获得|物品/.test(notice.text)?'item':/任务/.test(notice.text)?'quest':/奖励|文/.test(notice.text)?'reward':'tip';if(shownNoticeId===notice.id)return;shownNoticeId=notice.id;clearTimeout(noticeTimer);noticeTimer=setTimeout(()=>{notices.dismiss(notice.id);presentNotice();},notice.kind==='result'?2800:2100);}
...
function refreshShopPanel(){
  ...
  action.onclick=()=>void run(()=>controller.trade(side,selected.id,controller.shopQuantities[selected.id]??1));
  detail.append(icon,name,description,owned,price,controls,total,action);
  layout.append(list,detail);shopItems.append(layout);shopFeedback.textContent=controller.message;
}
