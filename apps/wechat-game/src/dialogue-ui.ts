...
export class MobileDialogueUi {
  ...
  constructor(scene: Phaser.Scene, width: number, height: number, insets: { left: number; right: number; bottom: number }, advance: () => void) {
    ...
    this.background = scene.add.rectangle(left, top, boxWidth, boxHeight, Number.parseInt(uiTokens.colors.dialoguePanel.slice(1), 16), .96)
      .setOrigin(0).setStrokeStyle(2, Number.parseInt(uiTokens.colors.accentSoft.slice(1), 16)).setDepth(PORTRAIT_DEPTH + 5).setVisible(false).setInteractive();
    this.background.on('pointerdown', advance);
    this.nameplate = scene.add.text(left + 24, top - 26, '', {
      fontFamily: 'Microsoft YaHei, Arial', fontSize: `${mobileTypography.speaker}px`, color: uiTokens.colors.dialogueBody,
      backgroundColor: uiTokens.colors.speakerNpc, padding: { left: 15, right: 15, top: 6, bottom: 6 }
    ...
    this.body = scene.add.text(left + 28, top + 22, '', {
      ...
      wordWrap: { width: boxWidth - 56 }, lineSpacing: 5
    }).setDepth(PORTRAIT_DEPTH + 6).setVisible(false).setInteractive();
    this.body.on('pointerdown', advance);
    this.continueHint = scene.add.text(right - 20, bottom - 12, '▼', {
      fontFamily: 'Arial', fontSize: '18px', color: uiTokens.colors.dialogueBody
